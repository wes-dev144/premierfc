...
from database.tables.club import Club, ClubSchema
from database.tables.club_membership import ClubMember, UserClubSchema
...
class UserClubsList(Resource):
    def get(self, user_id):
        query = db.session.query(ClubMember, ClubMember._club_id, Club.name, Club.location, User.first_name, User.last_name
                        ).join(Club, Club._club_id == ClubMember._club_id
                        ).join(User, Club.owner_id == User._user_id).filter(ClubMember._user_id == user_id).all()
        json_return = {"clubs": []}
        for result in query:
            club_id = result[1]
            club_name = result[2]
